...
function CT_Timer_ModTime(self, num)
	...
	CT_Timer_SetTime(CT_Timer.time, self:GetParent());
end
...
function CT_TimerOptions_OnClick(id, checked)
	if ( id == 1 ) then
		...
		_G["CT_TimerOptionsFrameShowTimerCB"]:SetChecked(checked);
		if ( checked ) then
			CT_TimerFrame:Show();
		else
			CT_TimerFrame:Hide();
	...
	elseif ( id == 2 ) then
		...
		_G["CT_TimerOptionsFrameShowSecondsCB"]:SetChecked(checked);
		CT_Timer_SetTime(CT_Timer.time, CT_TimerFrame);
	elseif ( id == 3 ) then
		CT_Timer.hideBG = checked;
		_G["CT_TimerOptionsFrameHideBackgroundCB"]:SetChecked(checked);
		CT_Timer.alphaUpdate = 5; -- Force redraw
	end
end
...
module.frame = function()
	...
	options["frame#tl:0:-" .. yoffset .. "#br:tr:0:-".. (yoffset + ysize)] = {
		...
		"font#t:0:-25#s:0:30#l:13:0#r#You can use /tr, or /timer, to display a list of commands in the chat window.#0.6:0.6:0.6:l",
	};
	...
	options["frame#tl:0:-" .. yoffset .. "#br:tr:0:-".. (yoffset + ysize)] = {
		...
		"font#t:5:-25#s:0:30#l:13:0#r#Click the button below to toggle the CT_Timer window open/closed.#0.6:0.6:0.6:l",
		["button#t:0:-70#s:120:30#v:GameMenuButtonTemplate#Toggle timer"] = {
			["onclick"] = function(self)
				if (CT_TimerFrame:IsShown()) then
					CT_TimerFrame:Hide();
				else
					CT_TimerFrame:Show();
				end
		...
		"font#t:5:-120#s:0:30#l:13:0#r#Click the button below to open the CT_Timer options window.#0.6:0.6:0.6:l",
		["button#t:0:-165#s:120:30#v:GameMenuButtonTemplate#Show options"] = {
			["onclick"] = function(self)
				ShowUIPanel(CT_TimerOptionsFrame);
			end,
		},
	...
	yoffset = yoffset + ysize + 20;
 	options["frame#tl:0:-" .. yoffset .. "#r"] = {
		"font#tl:5:0#v:GameFontNormalLarge#Commands",
	};
	yoffset = yoffset + 20;
	for i, text in ipairs(CT_TIMER_HELP) do
		tinsert(options, "font#t:5:-" .. yoffset .. "#s:0:40#l:13:0#r#" .. text .. "#0.6:0.6:0.6:l");
		yoffset = yoffset + 40;
	end
