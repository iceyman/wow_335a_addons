...
-- ----------
CT_RA_MTPlayers_cVersion = "3.301";
...
function CT_RA_MTPlayers_isCTRA(showErr)
	...
	-- --------------
	local ok = true;
	for i = 1, getn(CT_RA_MTPlayers_HookedFuncs) do
		local f = CT_RA_MTPlayers_HookedFuncs[i];
		if (f.script) then
			local frame = getglobal(f.name);
			if (not frame) then
				ok = false;
				break;
			end
...
function CT_RA_MTPlayers_OnLoad()
	...
	CT_RA_MTPlayers:RegisterEvent("RAID_ROSTER_UPDATE");
...
function CT_RA_MTPlayers_OnEvent(self, event, ...)
	...
	elseif (event == "RAID_ROSTER_UPDATE") then
		-- When initially logging in GetNumRaidMembers() returns 0 even if you are in a raid.
...
function CT_RA_MTPlayers_Hook(name, script, new)
	...
	if (script) then
		...
		newObject = getglobal("CT_RA_MTPlayers_" .. new .. script);
		if (not newObject) then
			return false;
		end
	else
		newObject = getglobal("CT_RA_MTPlayers_" .. new);
		if (not newObject) then
			return false;
		end
	end
...
function CT_RA_MTPlayers_CT_RA_UpdateUnitBuffs(buffs, frame, nick)
	...
	-- If this person is a main tank, then update the buffs on their Main Tank Player frame.
	local num, val = 0;
	for i = 1, ( tempOptions["ShowNumMTs"] or 10 ), 1 do
		val = CT_RA_MainTanks[i];
		if ( val ) then
			num = num + 1;
			local frame = getglobal("CT_RAMTGroupPlayerUnitButton"..num);
			if ( frame ) then
				local raidid = SecureButton_GetUnit(frame);  -- frame.unit;
				if ( raidid ) then
...
--							end
							CT_RA_MTPlayers_EK_RA_UpdateMTP(raidid, mtid, frame, key, val, false);
			...
			if (tempOptions["ctmtp_MTPets"] == 1) then
				local frame = getglobal("CT_RAMTGroupPetUnitButton"..num);
				if ( frame and frame:IsShown()) then
					local frameParent = frame.frameParent;
					local raidid = SecureButton_GetUnit(frame);  -- frame.unit;
					if ( raidid ) then
...
--[[
							if ( not isOnline ) then  -- UnitIsConnected(raidid) ) then
								...
								frame.ekisDead = isDead;
--							end
							CT_RA_MTPlayers_EK_RA_UpdateMTP(raidid, mtid, frame, key, val, true);
						elseif (alphaRange) then
							frame:SetAlpha(CT_RA_UnitAlpha(raidid, nil));
							CT_RA_UpdateRaidTargetIcon(frame, raidid);
						end
...
function CT_RA_MTPlayers_EK_RA_UpdateMTP(raidid, mtid, frame, key, val, isPet)
	-- ----------
	...
	CT_RA_MTPlayers_EK_RA_UpdateMT(raidid, mtid, frame, key, val, isPet);
...
function CT_RA_MTPlayers_EK_RA_UpdateMT(raidid, mtid, frame, key, val, isPet)
	-- ----------
...
-- &&			end
		...
		elseif ( health and healthmax and not UnitIsDead(raidid) and not UnitIsGhost(raidid) ) then
			...
			if ( showHP and showHP == 1 and maxHealth and memberHeight == 40 ) then
				frame.Percent:SetText(floor(percent/100*maxHealth) .. "/" .. maxHealth);
...
local function prepareFrame2(splitView, showmts, showmtplayers, showmtpets)
	...
	ancDrag = getglobal(CT_RA_MTPlayers_Groups[ancGrp] .. "Drag");
...
--	if (showmtplayers) then
		frame = prepareFrame(CT_RAMTGroupPlayer, CT_RAMTGroupPlayerDrag, "CT_RAMTMemberTemplate", CT_RA_SetupMTFrame, "MT Players", 2, "", showmts, showmtplayers, showmtpets);
		frame:SetAttribute("nameList", list);
--	end

--	if (showmtpets) then
		frame = prepareFrame(CT_RAMTGroupPet, CT_RAMTGroupPetDrag, "CT_RAMTMemberTemplate", CT_RA_SetupMTFrame, "MT Pets", 3, "pet", showmts, showmtplayers, showmtpets);
		frame:SetAttribute("nameList", list);
--	end
...
function CT_RA_MTPlayers_CT_RAMTGroupDragOnMouseDown(self, button)
	...
	elseif (button == "RightButton") then
		if (mtdrag or mtmenu) then
			...
			local oDrop = getglobal("CT_RA_MTPlayers_DropDown");
...
function CT_RA_MTPlayers_CT_RA_UpdateRaidMovability()
	...
	if ( tempOptions["LockGroups"] or (not showmtplayers) ) then
		getglobal("CT_RAMTGroupPlayerDrag"):Hide();
	else
		for i = 1, 10, 1 do
			if ( CT_RA_MainTanks[i] ) then
				CT_RAMTGroupPlayerDrag:Show();
	...
	if ( tempOptions["LockGroups"] or (not showmtpets) ) then
		getglobal("CT_RAMTGroupPetDrag"):Hide();
	else
		for i = 1, 10, 1 do
			if ( CT_RA_MainTanks[i] ) then
				CT_RAMTGroupPetDrag:Show();
...
--		member = getglobal("CT_RAMTGroupPlayerUnitButton"..y);
		if ( member ) then
			raidid = SecureButton_GetUnit(member);
			...
			if ( raidid and UnitExists(raidid) and CT_RA_Stats[CT_RA_MTPlayers_UnitName(raidid)] ) then
				CT_RA_MTPlayers_MTP_UpdateUnitBuffs(CT_RA_Stats[CT_RA_MTPlayers_UnitName(raidid)]["Buffs"], member, CT_RA_MTPlayers_UnitName(raidid));
			end
		end
		member = CT_RAMTGroupPet:GetAttribute("child".. y);
--		member = getglobal("CT_RAMTGroupPetUnitButton"..y);
		if ( member ) then
			if ( not member.status ) then
				member:SetBackdropColor(defaultColor.r, defaultColor.g, defaultColor.b, defaultColor.a);
			end
