...
do
	...
	local function mailboxOpened()
		if (module.opt.openAllBags) then
			-- Game handles opening of backpack if not already open.
			-- Open bags (1 to 4)
			-- Deliberately opening backpack as well. One person reported that
			-- their backpack didn't open, even though the game calls OpenBackpack()
			-- when MAIL_SHOW event is received.
			for i = 0, 4 do
				OpenBag(i);
			end
		end
	...
	local function mailboxClosed()
		if (module.opt.closeAllBags) then
			-- Close backpack (0) and bags (1 to 4)
			for i = 0, 4 do
				CloseBag(i);
			end
		end
